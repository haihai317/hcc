...
<script>
	...
	import { getClaAndBr,addGoods } from '../../api/api';
	...
	export default {
		...
		　　},
			...
			onSubmit() {
				// 获取商品详情
				this.getUEContent();
				// console.log(this.form.paramArr);return false;
				// 商品主图不能为空
				...
				for (var i = 0; i < this.form.paramArr.length; i++) {
					if(this.form.paramArr[i].param_title == "" || this.form.paramArr[i].param_value == ""){
						this.$message({
							message: '商品参数的标题或参数值不能为空',
							type: 'warning'
						});
						return false;
					}
